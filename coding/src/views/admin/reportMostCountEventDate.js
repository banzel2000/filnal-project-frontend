...
export default function ReportMostCountEventDate() {
  ...
  const [mostEventCountDateList, setMostEventCountDateList] = useState([]);
  ...
  async function queryMostEventCountDate() {
    let response = await Axios.post(ROOT_SERVER+"/queryMostEventCountDate", { 
      event_role_id: 0,
    });
    console.log(response);
    if (response.data.status === 1) { 
          setMostEventCountDateList(response.data.mostEventCountDate); 
    }
  }
            ...
            <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
              <div className="rounded-t bg-white mb-0 px-3 py-3">
                <div className="text-center flex justify-between ">
                  ...
                  รายงานจำนวนกิจกรรมที่จัดตั้งยาวนาน/สั้น ที่สุด{"("+mostEventCountDateList.length+")"}
                  </h3>
                  <p>
                    <InputGroup size="sm">
                      <Form.Select
                  ...
                  <table className="items-center w-full bg-transparent border-collapse">
                    <thead>
                      <tr>
                        ...
                        <th className="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left bg-blueGray-50 text-blueGray-500 border-blueGray-100">
                          จำนวนการจัดจั้งกิจกรรม
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {mostEventCountDateList.length <= 0 ? (
                        <td
                          ...
                          .map((val, key) => {
                            return (
                              <tr>
                                ...
                                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-1">
                                 {val.event_count_date == 0 ? "ยังไม่ถึง 1 วัน" : val.event_count_date }
                                </td>
                              </tr>
                            );
                          })
